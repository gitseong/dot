		...
		if(!error && response.statusCode === 200) {
			...
			for(let i = 0; i < JSON.parse(body).results.length; i++) {
				searchCategoryArray.push(
					{
						placeIndex : i,
						placeId : JSON.parse(body).results[i]['place_id'],
						placeName : JSON.parse(body).results[i].name,
						placeOpeninghours : JSON.parse(body).results[i]['opening_hours'],
						placeRating : JSON.parse(body).results[i].rating,
						placeTypes : JSON.parse(body).results[i].types,
						placeDetailAddress : JSON.parse(body).results[i].vicinity
					}	
				);
